	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($user_name) && !empty($password))
		{
			...
			if($result)
			{
				...
				$queryadmin = "select * from adminn where Admin_username = '$user_name' limit 1";
				...
				if($resultadmin){ 
					...
					{ 
						...
						if($Admin_data['Admin_password'] === $password)
						{
							...
							if($Admin_data['A_code'] == 'Owner')
							{
								header("Location: Owner.php");
								die;	
							}
							else if($Admin_data['A_code'] == 'Manager')
							{
								header("Location: Manager.php");
								die;	
							}
							else if($Admin_data['A_code'] == 'Financial officer')
							{
								header("Location: FinancialOfficer.php");
								die;	
							}
							else if($Admin_data['A_code'] == 'Delivery manager')
							{
								header("Location: DeliveryManager.php");
								die;	
							}
							else if($Admin_data['A_code'] == 'Driver')
							{
